...
class AutoEncoderTrainer(BaseTrainer):
    ...
    def train_epoch(self, epoch):
        ...
        for step, batch in progress_bar:
            ...
            # TRAINING LOOP of autoencoder (no noise)
            reconstructed_signals = self.model(signals)
            loss = F.mse_loss(reconstructed_signals, signals)
    ...
    def val_epoch(self, epoch):
        ...
        for step, batch in progress_bar:
            ...
            # VALIDATION LOOP of autoencoder (no noise)
            reconstructed_signals = self.model(signals)
            loss = F.mse_loss(reconstructed_signals, signals)
